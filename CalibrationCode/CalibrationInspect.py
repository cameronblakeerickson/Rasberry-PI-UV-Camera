...
#distancesfname="BeforeRing4sec.dng"


fname_dist=["No_Ring_0.1sec.dng","After_Ring_0.1sec.dng"]
fname_exposures=[0.1,0.1]
...
countour_plot_pts=[]
sds=[]
titles=["Beam + focusing lens Without Ring", "Beam+ focusing lens After Ring"]
#/Users/blakeerickson/Library/CloudStorage/OneDrive-Personal/BlakesOldMacBook/Academics/PSI/UVcamera/Rasberry-PI-UV-Camera/Pictures/Uncoated_Bot_Ring/FirstLook/NoRing_2sec.dng
for i in range(len(fname_dist)):
    fname=fname_dist[i]
    print(fname)
    file_path = os.path.normpath(
    os.path.join(script_dir, "..", "Pictures", "Uncoated_Bot_Ring", "Transmission_Effeciency", fname))
    ...
    contours=CircularBlur.get_contours(image,0.2)#, hierarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
print("Effeciency:",Total_Powers[-1]/Total_Powers[0])
print(Total_Powers);quit()
#plt.show()
#quit()
fig, ax = plt.subplots(figsize=(5,5))
labels=["Without Ring","With Uncoated Ring"]
areas=[]
dx=[]
dy=[]
